...
1e. An INVESTIGATIVE / EXPLANATORY wrapper does NOT change WHICH metric is asked. Questions of the form "WHY did <measure> <change-word>", "WHAT DROVE / caused / is behind / explains <measure>", "explain the <change> in <measure>", or "BREAK DOWN <measure>" ask about the SAME governed metric the plain lookup ("what was <measure>") would resolve — the explanatory framing (and any change-word: drop, fall, decline, dip, decrease, shrink, fall off, spike, jump, surge, climb, rise, grow, change, move, swing) is NOT itself a metric. Strip the wrapper, resolve the underlying MEASURE, and classify by that measure ALONE: if the measure IS governed -> "answered" with that metric (a downstream loop then investigates the movement); if the measure is NOT governed -> decline EXACTLY as you would the bare lookup. This NEVER converts a non-governed derived measure into a base metric: "recurring revenue", "revenue growth rate", "refund rate", "profit margin", "cache hit rate" STAY "unknown_metric" whether or not a why/what-drove wrapper is present — a change-word like "grow"/"rise" is framing, not permission to answer a governed-absent rate/ratio/%/growth measure. A why/what-drove wrapper is explanation, NOT a superlative, so set "rank" to null for it.
...
"Why did net revenue drop last month?" -> {{"classification":"answered","metrics":["total_net_revenue"],"dimensions":[],"time_range":"last month","domain":"Revenue","ambiguous":false,"filters":[],"rank":null}}  (investigative wrapper "why did ... drop" over a GOVERNED metric -> SAME metric as the plain lookup; the loop then investigates the drop)
"What drove total token usage last week?" -> {{"classification":"answered","metrics":["total_tokens_consumed"],"dimensions":[],"time_range":"last week","domain":"Token usage","ambiguous":false,"filters":[],"rank":null}}  ("what drove <metric>" -> strip the wrapper, resolve the governed metric)
"Why did recurring revenue drop last month?" -> {{"classification":"unknown_metric","metrics":[],"dimensions":[],"time_range":"last month","domain":"Revenue","ambiguous":false,"filters":[],"rank":null}}  (the wrapper does NOT rescue an UNGOVERNED measure — recurring revenue stays unknown; the loop short-circuits)
